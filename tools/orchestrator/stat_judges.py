...
STAT_CHECKS: tuple[StatCheck, ...] = (
    ...
    StatCheck(
        ...
        rubric=(
            ...
            "computations? "
            "FAIL only when ALL THREE hold: (a) a summary function (R: sum, mean, "
            "var, sd, min, max; Python: np.sum, df.mean, etc.) is called on a "
            "variable that plausibly originates from real-world data with missing "
            "values, AND (b) no NA guard is present (na.rm=TRUE, na.omit, "
            "complete.cases, dropna, fillna, skipna=True), AND (c) there is no "
            "documentation or code comment stating the data are complete. "
            "IMPORTANT - do NOT fail for these patterns: "
            "(1) Simulation or synthetic data: if the data are generated by rnorm(), "
            "runif(), rbinom(), sample(), or equivalent - NAs cannot arise, so "
            "na.rm is unnecessary. "
            "(2) Global guard applied upstream: if na.omit() / complete.cases() / "
            "dropna() is applied to the dataset before the aggregation, the "
            "individual calls need not repeat na.rm=TRUE. "
            "(3) Model calls: lm(), glm(), and most R modelling functions apply "
            "na.action=na.omit by default - do not fail for missing na.rm in a "
            "modelling context. "
            "(4) Ambiguous origin: if you cannot determine from the evidence alone "
            "whether the variable can contain NAs, mark unverified rather than fail. "
            "PASS if na.rm=TRUE is consistently present, or if a global omit/drop "
            "is applied, or if the data are synthetic or documented as complete. "
